...
begin
  ...
  try
    case Task of
      TASK_VIEW_ACC              :
        begin
          FillInvoiceList(qryAccepted);

          if SiteCode <> 0 then
          begin
            qryAccByDate.Filtered := true;
            qryAccByDate.Filter := '[Site Code] = ' + IntToStr(SiteCode);
          end;
          qryAccByDate.open;
        end;
...
begin
  ...
  try
    if isMaster and (Task = TASK_VIEW_ACC) then  // and SiteCode <> 0
    begin
      if orderField = 'Date' then // use special query
      begin
        dsInvoiceList.DataSet := qryAccByDate;
        suppSearch.SearchField := 'Date';
        suppSearch.FindValue;
      end
      else   // use original query
      begin
        dsInvoiceList.DataSet := qryAccepted;
    ...
    on E: Exception do
    begin
      Log.Event('fgetinvdlg; ERROR - SetGUISortDetails: ' + E.Message +
         ' , ' + dsInvoiceList.DataSet.Name);
      raise;
    end;
  end;
end;
  ...
  case Task of
    TASK_VIEW_ACC :
      begin
        ...
        qryAccByDate.FieldByName('Site Code').Visible := false;
        qryAccByDate.FieldByName('Supplier Name').ReadOnly := true;
        qryAccByDate.FieldByName('Supplier Name').DisplayLabel := 'Supplier';
        qryAccByDate.FieldByName('Supplier Name').DisplayWidth := 20;

        qryAccByDate.FieldByName('Delivery Note No.').ReadOnly := true;
        if (UKUSmode = 'UK') then
          qryAccByDate.FieldByName('Delivery Note No.').DisplayLabel := 'Delivery Note No.'
        else
          qryAccByDate.FieldByName('Delivery Note No.').DisplayLabel := 'Invoice No.';
        qryAccByDate.FieldByName('Delivery Note No.').DisplayWidth := 15;

        qryAccByDate.FieldByName('Note').Visible := false;

        qryAccByDate.FieldByName('Date').ReadOnly := true;
        qryAccByDate.FieldByName('Date').DisplayWidth := 10;

        qryAccByDate.FieldByName('Order No').Visible := false;

        qryAccByDate.FieldByName('Total Amount').ReadOnly := true;
        qryAccByDate.FieldByName('Total Amount').DisplayLabel := 'Total Cost';
        qryAccByDate.FieldByName('Total Amount').DisplayWidth := 10;
        TFloatField(qryAccByDate.FieldByName('Total Amount')).DisplayFormat := '0.00';
      end;
