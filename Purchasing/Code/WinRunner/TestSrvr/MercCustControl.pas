(* $Id: MercCustControl.pas,v 1.15 1999/08/23 10:19:28 yuv Exp $ [MISCCSID] *)
{
...
interface
uses
 Windows,Messages, SysUtils,TypInfo, Controls,Forms,MercControl,
 Grids, DBGrids, Db, DBTables,Mask,StdCtrls,DBCtrls;
...
begin
     ...
     else if (MercUnitManger.InheritsFrom(Obj,'TPanel') or MercUnitManger.InheritsFrom(Obj,'TToolbar97')) then
...
function  TMercGrid.Get_Recording() :string ;
  ...
  if ( (Msg.Msg=WM_LBUTTONUP) or
     ...
     if (Length(col_str)=0) then col_str:='#'+inttostr(col);
      ...
      if Assigned(TDBGrid(FObject).DataSource) then
       ...
       // if ( dgIndicator in TDBGrid(FObject).Options ) then
          if ( row = 0 ) then
            ...
            TDBGrid(FObject).DataSource.Dataset.DisableControls;
            TDBGrid(FObject).DataSource.Dataset.First;
            TDBGrid(FObject).DataSource.Dataset.MoveBy(Integer(row - 1));
...
begin
 Result :='';
 Control:=TWinControl(FObject).ControlAtPos(p,TRUE);
...
for i:=0 to TWinControl(FObject).ControlCount-1 do
 begin
 Control:=TWinControl(FObject).Controls[i];
