...
(*
  Todo:
    ...
    Would be nice to fix the indices for levels - currently are 0 based in
    code but 1 based in SQL data.
*)
...
type
  ...
  public
    constructor Create(SourceTree: TTreeView; Connection: TAdoConnection; DataTable: string; var NamesArray: TNamesArray; PartialLevels: boolean = False);
...
constructor TDataTree.Create(SourceTree: TTreeView; Connection:TADOConnection; DataTable: string; var NamesArray: TNamesArray; PartialLevels: boolean = False);
begin
  inherited Create;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        NewNode := Nodes.AddNode(nil, Node, NamesArray[DataSet.FieldByName('Name').AsInteger], TObject(NodeData), naAddChild);
        NewNode.HasChildren := NewNodesHaveChildren;
        NewNode.ImageIndex := NodeImageIndex;
        NewNode.SelectedIndex := NodeImageIndex;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          NewLevel := i;
          NodeText := NamesArray[ADOQuery.FieldByName(Format('Level%dname', [NewLevel+1])).AsInteger];
