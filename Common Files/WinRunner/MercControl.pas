(* $Id: MercControl.pas,v 1.25 2003/05/04 11:06:38 taiga Exp $ [MISCCSID] *)
{
...
TMercControl = class (TControl)
  ...
  public
		...
		function  GetNotSupportProperty(obj:TObject;propname:string): String;
        function  GetObjectPropertyEx(obj:TObject;propname:string; var bRes: BOOL): string;
        ...
        function  GetProperty(propname:string; var bRes: bool): string; virtual;
...
idx:=pos('[',propname);
 ...
 try
 begin
  if(MercUnitManger.InheritsFrom(Obj,'TMenuItem')) then
  begin
  	if(UpperCase(propname)='ITEMS') then
    begin
		if(n >= 0) and (n < TMenuItem(Obj).Count) then
	      	Result:= TMenuItem(Obj).Items[n]
        else
        	Result:=nil;
    end
  end
  else if(MercUnitManger.InheritsFrom(Obj,'TToolBar')) then
  begin
   	if(UpperCase(propname)='BUTTONS') then
    begin
		if(n >= 0) and (n < TToolBar(Obj).ButtonCount) then
	      	Result:= TToolBar(Obj).Buttons[n]
        else
        	Result:=nil;
    end
  end
  else if (UpperCase(propname)='PARENT') then Result:=TWinControl(Obj).parent
...
function  TMercControl.GetNotSupportProperty(obj:TObject;propname:string): String;
var
    idx,idx2,n,ItemCount:Integer;
    s:string;
begin
	Result:='';
    n:=-1;
    ...
    try
		if(MercUnitManger.InheritsFrom(Obj,'TMenuItem')) then
		begin
		  	if(UpperCase(propname)='COUNT') then
				str(TMenuItem(Obj).Count, Result);
		end
        else if(MercUnitManger.InheritsFrom(Obj,'TToolBar')) then
		begin
		  	if(UpperCase(propname)='BUTTONCOUNT') then
				str(TToolBar(Obj).ButtonCount, Result);
		end
        else if(MercUnitManger.InheritsFrom(Obj,'TCheckListBox')) then
        begin
            if  (UpperCase(propname)='CHECKED') and (n >= 0) then
             begin
                {$IFDEF VER140}
                // for delphi 6
                ItemCount := TCheckListBox(Obj).Count;
                {$else}
                // For Delphi 3,4,5
                ItemCount := TCheckListBox(Obj).Items.Count;
                {$endif}
                 
                if(n < ItemCount) then
                begin
                    if(TCheckListBox(Obj).Checked[n]) then
                        Result:= 'true'
                    else
                        Result:= 'false';
                end
        ...
        else if(MercUnitManger.InheritsFrom(Obj,'TWinControl')) then
        begin
            if (UpperCase(propname)='CONTROLCOUNT') then
                str(TWinControl(Obj).ControlCount, Result);
        end;
...
// Overloaded for internal use - parameter var bRes: BOOL
function  TMercControl.GetObjectProperty(obj:TObject;propname:string): string;
var bRes: BOOL;
begin
  bRes:= FALSE;
  Result:= GetObjectPropertyEx(obj, propname, bRes);
end;


function  TMercControl.GetObjectPropertyEx(obj:TObject;propname:string; var bRes: BOOL): string;
...
begin
    ...
    if (PropInfo = nil) then
        begin
             ...
             if (ObjSub = nil) then
             begin
             	Result:= GetNotSupportProperty(obj, propname);
                exit;
             end;

             Result :=GetObjectPropertyEx(ObjSub,PropSubName, bRes);
             exit;
        end;
    ...
    case PropInfo^.PropType^.Kind of
          ...
          tkClass:
            begin
             ...
             begin
	             FObject:= ObjSub;
	             Result := GetProperty(PropSubName, bRes);
             end;
             exit;
            end;
          end;
...
function TMercControl.GetProperty(propname:string; var bRes: BOOL): string;
begin
GetParamFromProperty(propname);
Result:='';
bRes := FALSE;
if (not IsTControlProperty(propname)) then
   Result :=GetObjectPropertyEx(self, propname, bRes);
if (Result = '') then
   Result :=GetObjectPropertyEx(FObject, propname, bRes);
end;
