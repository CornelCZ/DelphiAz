{------------------------------------------------------------------------------}
{ Copyright © 2000-2005, Simon Armstrong.  All Rights Reserved.                }
{                                                                              }
{ Copyright:                                                                   }
...
{ SadMan Software web site (http://www.simes.clara.co.uk) is distributed       }
{ unmodified.                                                                  }
{                                                                              }
{ Restrictions:                                                                }
...
{                                                                              }
{ Support:                                                                     }
{ Support is provided via email to Support@simes.clara.co.uk. However, I can   }
{ not guarantee any support whatsoever. While I do try to answer all questions }
{ that I receive and address all problems that are reported to me, you must    }
{ understand that I simply can not guarantee that this will always be so.      }
{                                                                              }
{ History:                                                                     }
{ June 2005 Added option to limit extent of move/resize to be within the       }
...
type
  ...
  TControlSizer = class(TComponent)
  private
    FAllowMove: boolean;
  ...
  published
    ...
    property Control: TControl read FControl write SetControl;
    property LimitToParentRect: boolean read FLimitToParentRect write FLimitToParentRect default false;
...
procedure Register;
begin
  RegisterComponents('SadMan', [TControlSizer]);
end;
...
const
  HandleCursors: array[TGrabHandlePosition] of TCursor = (
    crSizeNWSE, crSizeNS,
    crSizeNESW, crSizeWE,
    crSizeNWSE, crSizeNS,
    crSizeNESW, crSizeWE);
...
procedure TControlSizer.DoSubclass;
begin
...
procedure TControlSizer.DrawMoveRect(const sx, sy: integer);
var
  l, t: integer;
  rct: TRect;
begin
...
procedure TControlSizer.DrawSizeRect(Sender: TObject; sx, sy: integer);
var
  rct: TRect;
begin
  if not Assigned(FControl) or
...
procedure TControlSizer.EndMoveControl(const x, y: Smallint);
var
  l, t: integer;
  rct: TRect;
begin
...
procedure TControlSizer.FreeCanvas;
var
  h: THandle;
begin
...
procedure TControlSizer.GetCanvas;
var
  h: THandle;
begin
  h := 0;
...
function TControlSizer.GetControlBoundsRect(Sender: TObject; sx, sy: integer): TRect;
var
  pt: TPoint;
begin
...
function TControlSizer.LimitPointToParent(const pt: TPoint): TPoint;
begin
  result := pt;
...
function TControlSizer.LimitRectToParent(const rct: TRect): TRect;
begin
  result := rct;
...
procedure TControlSizer.MoveControl(const x, y: Smallint);
begin
...
procedure TControlSizer.Notification(AComponent: TComponent; AOperation: TOperation);
begin
  inherited Notification(AComponent, AOperation);
  if (AComponent = FControl) and (AOperation = opRemove) then
    Control := nil;
...
procedure TControlSizer.OnDragHandle(Sender: TObject; sx, sy: integer);
begin
  DrawSizeRect(Sender, sx, sy);
end;

procedure TControlSizer.OnEndDragHandle(Sender: TObject; sx, sy: integer);
var
  rct: TRect;
begin
...
procedure TControlSizer.OnStartDragHandle(Sender: TObject; sx, sy: integer);
begin
...
procedure TControlSizer.ReSetHandles;
var
  h: TGrabHandlePosition;
begin
  for h := low(TGrabHandlePosition) to High(TGrabHandlePosition) do begin
...
procedure TControlSizer.SetAllowMove(const v: boolean);
begin
...
procedure TControlSizer.SetAllowResize(const v: boolean);
begin
...
procedure TControlSizer.SetControl(const c: TControl);
var
  h: TGrabHandlePosition;
begin
  if csDestroying in ComponentState then exit;
...
procedure TControlSizer.SetVisible(const v: boolean);
var
  h: TGrabHandlePosition;
begin
  for h := low(TGrabHandlePosition) to high(TGrabHandlePosition) do
...
procedure TControlSizer.StartMoveControl(const x, y: Smallint);
begin
...
procedure TControlSizer.SubclassProc(var msg: TMessage);
begin
...
procedure TControlSizer.UndoSubclass;
begin
