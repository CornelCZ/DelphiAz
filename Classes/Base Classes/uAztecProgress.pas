...
type
  ...
  TAztecProgressBar = class(TCustomControl)
  private
...
Procedure Register;
begin
  RegisterComponents('Aztec Components', [TAztecProgressBar]);
end;


Procedure TAztecProgressBar.InitBlockArray;
...
Procedure TAztecProgressBar.InitPixArray;
...
Function TAztecProgressBar.MakeCylinder(h:real):extended;                      // NIH
...
Function TAztecProgressBar.GetGradientAr2(aColor:TColor;sz:Integer):TClrArray;
...
function TAztecProgressBar.RGBtoHLS(RGBColor: TColor): THLSRec;              // NIH
...
function TAztecProgressBar.HLStoRGB(hue,lum,sat: THLSRange): TColor;         // NIH
// (c) Microsoft. http://support.microsoft.com/default.aspx?scid=kb;en-us;29240
var
...
function TAztecProgressBar.GetColorBetween(StartColor, EndColor: TColor; Pointvalue,
...
procedure TAztecProgressBar.Paint;
...
constructor TAztecProgressBar.Create (AOwner : TComponent);
begin
...
destructor TAztecProgressBar.Destroy;
begin
  fFont.Free;
...
Procedure TAztecProgressBar.LockInits;
Begin
  self.fInitsLocked := True;
End;


Procedure TAztecProgressBar.UnlockInits;
Begin
...
procedure TAztecProgressBar.Resize;
Begin
  inherited Resize;
...
procedure TAztecProgressBar.SetUsefullWidth;
Begin
  Case Self.fOrientation of
...
procedure TAztecProgressBar.SetFCorner(isRounded:Boolean);
Begin
...
function TAztecProgressBar.GetBoolCorner : Boolean;
Begin
  result := Self.fCorner > 0;
End;


procedure TAztecProgressBar.SetBarKind(value:TQBarKind);
Begin
  Self.fBarKind := value;
  InitPixArray;
  Invalidate;
...
procedure TAztecProgressBar.SetBarLook(value:TQBarLook);
Begin
  self.fBarLook := value;
  InitPixArray;
  Invalidate;
...
procedure TAztecProgressBar.SetOrientation(value:TQBarOrientation);
var tmpClr:TColor;
    newH,
    newW:Integer;
Begin
...
procedure TAztecProgressBar.SetBkgColor(aColor:TColor);
Begin
  Self.fBkgClr := aColor;
  Invalidate;
End;
...
procedure TAztecProgressBar.SetShape(value:Boolean);
Begin
...
procedure TAztecProgressBar.SetShapeColor(value:TColor);
Begin
...
procedure TAztecProgressBar.SetBlockSize(value:Integer);
Begin
  Case Self.fOrientation of
...
procedure TAztecProgressBar.SetSpaceSize(value:Integer);
Begin
  Case Self.fOrientation of
    boHorizontal : If value > Self.Width  - (fInternalBorder SHL 1) Then Exit;
    boVertical   : If value > Self.Height - (fInternalBorder SHL 1) Then Exit;
...
procedure TAztecProgressBar.SetFullBlock(value:Boolean);
Begin
...
procedure TAztecProgressBar.SetMaxPos(value:Integer);
Begin
...
procedure TAztecProgressBar.SetPosition(value:Integer);
var tmpfPos : real;
Begin
  fUserPos := value;
...
procedure TAztecProgressBar.SetStartClr(value:TColor);
Begin
...
procedure TAztecProgressBar.SetFinalClr(value:TColor);
Begin
...
procedure TAztecProgressBar.SetBothColors(value:TColor);
Begin
  Self.fMonoClr  := True;
  Self.fStartClr := value;
  Self.fFinalClr := value;
...
procedure TAztecProgressBar.SetInactivePos(value:Boolean);
Begin
...
procedure TAztecProgressBar.SetInactPosClr(value:TColor);
Begin
...
procedure TAztecProgressBar.SetInvInactPos(value:Boolean);
...
procedure TAztecProgressBar.SetCaption(value:String);
Begin
  Self.fCaption := Value;
...
procedure TAztecProgressBar.SetCapAlign(value:TAlignment);
Begin
...
procedure TAztecProgressBar.SetCaptionOvr(value:Boolean);
Begin
...
procedure TAztecProgressBar.SetFont(value:TFont);
Begin
