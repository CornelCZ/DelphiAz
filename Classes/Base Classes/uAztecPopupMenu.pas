...
type
  TAztecPopupMenu=class(TPopupMenu)
  private
    FFont:TFont;
    FColor:TColor;
...
procedure Register;
begin
  RegisterComponents('Aztec Components', [TAztecPopupMenu]);
end;

{ TAztecPopupMenu }

procedure TAztecPopupMenu.AssignDrawMethods(MenuItem:TMenuItem);
var
  Index:integer;
begin
  try
...
constructor TAztecPopupMenu.Create(AOwner: TComponent);
begin
  inherited;
  FFont:=TFont.Create;
  FFont.Name:='Arial';
...
destructor TAztecPopupMenu.Destroy;
begin
  FreeAndNil(FFont);
...
procedure TAztecPopupMenu.DrawMenuItem(Sender: TObject; ACanvas: TCanvas;
  ARect: TRect; State: TOwnerDrawState);

  procedure RemoveMenuBorder;
  var
    TempCanvas:TCanvas;
    MenuWindowHandle:HWND;
    MenuItemRect:TRect;
  begin
...
procedure TAztecPopupMenu.MeasureMenuItem(Sender: TObject;
  ACanvas: TCanvas; var Width, Height: Integer);
begin
  Inc(Width, ACanvas.TextWidth(TMenuItem(Sender).Caption)+15);
end;

procedure TAztecPopupMenu.MenuChange(Sender: TObject; Source: TMenuItem;
  Rebuild: Boolean);
begin
...
procedure TAztecPopupMenu.SetFont(const AFont: TFont);
begin
  FFont.Assign(AFont);
end;
