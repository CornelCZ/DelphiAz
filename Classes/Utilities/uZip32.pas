...
// PW: 25/06/01 Fixed problem - application.processmessages was being called
//   unnecessarily throughout the zip/unzip process which could cause some
//   sequence-related bugs.


uses
  Forms, WinTypes, WinProcs, SysUtils, Classes, Messages, Dialogs, Controls,
  ShellApi, Graphics, Buttons, StdCtrls, uSystemUtils;

type
  TLogEvent = procedure(text: string) of object;
{Function-based interface for usage of ZIP files for non-visual tasks.}
{The "logfn" parameter should have a suitable logging method passed to it, or}
{nil if not required.}

{ArchiveFiles - Takes the list of full pathnames in "fileslist" and adds them}
{to the specified ZIP file.}
{if the archive already exists, files are added. if individual files are}
{already present in the archive, they will be updated.}
function ArchiveFiles(filelist: TStringlist; zipfile: string; logfn: TLogEvent): boolean;
{UnArchive - Unarchives the specified ZIP file to "destinationfolder".}
{if files already exist, they will NOT be overwritten.}
function UnArchive(zipfile:string; destinationfolder: string; logfn: TLogEvent): boolean;
function MultiArchiveFiles(directory: string; zipfile: string; logfn: TLogEvent): boolean;

{PW: This is a concatenation of all TZipMaster source into one unit, hardwired}
{for Delphi 5 (data from unit "zipvers")}
{All original documentation has been left in.}

{TZipMaster has been removed, and replaced with a non-vcl component, }

(* TZipMaster VCL by Chris Vleghert and Eric W. Engler  v1.60L   Feb 3, 2000.
 ...
 -Document ZipDirEntry structure for all fields.
...
{ ZCallBck}
const { Maximum no. of files in a single ZIP file }
  FilesMax = 4096;
...
{ ZipMsg}
  ...
  ZIPMASTERVERSION: string = '1.60 L';
...
{ These records are very critical.  Any changes in the order of items, the
  ...
  ZipParms2 = packed record
   ...
   // After this point the record is different from ZipParms1 structure.
    ...
    fNotUsed: array[0..13] of Cardinal; // NEW
...
// An entry in the central dir:
  ...
  protected
    procedure PwdOkClick(Sender: TObject); virtual;

  public
    OkClicked: Boolean;

    constructor CreateNew(Owner: TComponent{$IFDEF VERD4+}; Dummy: Integer{$ENDIF}); {$IFDEF VERD4+} override; {$ENDIF}
...
//------------------------------------------------------------------------
  ...
  AddStoreSuffixEnum = (assGIF, assPNG, assZ, assZIP, assZOO, assARC,
    assLZH, assARJ, assTAZ, assTGZ, assLHA, assEXE);
...
//  TMessageEvent = procedure(Sender: TObject; ErrCode: Integer; Message: string) of object;
  TMessageEvent = procedure(text: string) of object;
  ...
  TPasswordErrorEvent = procedure(Sender: TObject; var NewPassword: string; ForFile: string; var RepeatCount: LongWord) of object;
  TCRC32ErrorEvent = procedure(Sender: TObject; ForFile: string; FoundCRC, ExpectedCRC: LongWord; var DoExtract: Boolean) of object;
  TExtractOverwriteEvent = procedure(Sender: TObject; ForFile: string; IsOlder: Boolean; var DoOverwrite: Boolean) of object;
  ...
  private
    ...
    procedure ExtExtract(UseStream: Integer; InStream: TStream; IsData: Boolean);
  ...
  public
    ...
    //procedure ExtractFromStream( InStream: TStream; IsData: Boolean );
    function GetPassword(DialogCaption, MsgTxt: string; var ResultStr: string): Boolean;
    function CopyZippedFiles(DestZipMaster: TZip32; DeleteFromSource: boolean; OverwriteDest: OvrOpts): Integer;
  ...
  public
    constructor CreateFT(var myParent: TZip32; CreateSuspended: Boolean); virtual;
    destructor Destroy; override;
  end;
...
function EnumThreadWndProc(winh: HWND; var mythis: TZip32): Boolean; stdcall; export;
function CallWndRetProc(code: Integer; wp: WPARAM; lp: LPARAM): LRESULT; stdcall; export;

var
  SHFormatDrive: function(wnd: HWND; drive: Integer; size: Cardinal; action: Integer): Integer; stdcall;
...
var
  ...
  RptCount: LongWord;
begin
  with ZCallBackRec^, (TObject(Caller) as TZip32) do
  begin
...
//            FOnMessage(Caller, ErrorCode, Msg);
      ...
      8: { New or other password needed during Extract() }
        begin
          ...
          if Assigned(FOnPasswordError) then
            FOnPasswordError(Caller, pwd, Msg, RptCount)
      ...
      10: { Extract(UnZip) Overwrite ask }
        ...
        begin
          ...
          FOnExtractOverwrite(Caller, Msg, (ErrorCode and 1) = 1, DoOverwrite);
...
constructor TZip32.Create(AOwner: TComponent);
begin
  ...
  FAddStoreSuffixes := [assGIF, assPNG, assZ, assZIP, assZOO, assARC, assLZH, assARJ, assTAZ, assTGZ, assLHA];
...
procedure TPasswordDlg.PwdOkClick(Sender: TObject);
begin
  OkClicked := True;
end;
...
constructor TPasswordDlg.CreateNew(Owner: TComponent{$IFDEF VERD4+}; Dummy: Integer{$ENDIF});
begin
  inherited CreateNew(Owner{$IFDEF VERD4+}, Dummy{$ENDIF});
  Parent := Self;
  Width := 344;
  Height := 137;
  Font.Name := 'Arial';
...
procedure TZip32.ShowZipMessage(Ident: Integer; UserStr: string);
var
  Msg: string;
begin
...
//    FOnMessage(Self, 0, Msg); // No ErrCode here else w'll get a msg from the application
    FOnMessage(Msg); // No ErrCode here else w'll get a msg from the application
end;

function TZip32.LoadZipStr(Ident: Integer; DefaultStr: string): string;
begin
...
procedure TZip32.ShowExceptionError(const ZMExcept: EZipMaster);
begin
...
function TZip32.ConvCodePage(Source: string; Direction: CodePageDirection): string;
const
  FS_FAT: Integer = 0;
  FS_HPFS: Integer = 6;
  FS_NTFS: Integer = 11;
...
function TZip32.GetZipVers: Integer;
var
  AutoLoad: Boolean;
begin
  Result := 0;
...
function TZip32.GetUnzVers: Integer;
var
  AutoLoad: Boolean;
begin
  Result := 0;
...
procedure TZip32.SetFSpecArgs(Value: TStrings);
begin
  FFSpecArgs.Assign(Value);
end;

procedure TZip32.SetFSpecArgsExcl(Value: TStrings);
begin
  FFSpecArgsExcl.Assign(Value);
end;

procedure TZip32.SetSFXIcon(aIcon: TIcon);
begin
  FSFXIcon.Assign(aIcon);
end;

procedure TZip32.SetFilename(Value: string);
begin
  FZipFileName := Value;
...
procedure TZip32.SetDLLDirectory(Value: string);
var
  ValLen: Integer;
begin
...
procedure TZip32.SetRootDir(Value: string);
begin
  if Value <> FRootDir then
    FRootDir := AppendSlash(Value);
end;

function TZip32.GetCount: Integer;
begin
...
procedure TZip32.SetVersionInfo(Value: string);
begin
end;

procedure TZip32.SetPasswordReqCount(Value: LongWord);
begin
...
function TZip32.GetZipComment: string;
begin
  Result := ConvCodePage(FZipComment, cpdOEM2ISO);
end;

procedure TZip32.SetZipComment(zComment: string);
var
  EOC: ZipEndOfCentral;
  len: Integer;
  CommentBuf: pChar;
...
procedure TZip32.FreeZipDirEntryRecords;
var
  i: Integer;
begin
...
procedure TZip32.List; { all work is local - no DLL calls }
var
  pzd: pZipDirEntry;
  EOC: ZipEndOfCentral;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if FileRead(FInFileHandle, pzd^.ExtraData, CEH.ExtraLen) <> CEH.ExtraLen then
            raise EZipMaster.CreateResDisp(LI_ReadZipError, True);
        end
...
function TZip32.GetTotalFileSize(var ProgMax: Integer): LongInt;
var
...
procedure TZip32.AddSuffix(const SufOption: AddStoreSuffixEnum; var sStr: string; sPos: Integer);
const
  SuffixStrings: array[0..11, 0..3] of Char = ('gif', 'png', 'z', 'zip', 'zoo', 'arc', 'lzh', 'arj', 'taz', 'tgz', 'lha', 'exe');
begin
  if SufOption in fAddStoreSuffixes then
    sStr := sStr + '.' + string(SuffixStrings[sPos]) + ':';
end;

procedure TZip32.SetZipSwitches(var NameOfZipFile: string; zpVersion: Integer);
var
  i: Integer;
  SufStr: string;
...
procedure TZip32.SetDeleteSwitches; { override "add" behavior assumed by SetZipSwitches: }
begin
  with ZipParms.zp1^ do
  begin
    fDeleteEntries := True;
...
procedure TZip32.SetUnZipSwitches(var NameOfZipFile: string; uzpVersion: Integer);
begin
...
function TZip32.GetAddPassword: string;
var
  p1, p2: string;
begin
  p2 := '';
  if FUnattended then
    ...
    if GetPassword(LoadZipStr(PW_Caption, RESOURCE_ERROR), LoadStr(PW_MessageEnter), p1) and (p1 <> '') then
    begin
      if GetPassword(LoadZipStr(PW_Caption, RESOURCE_ERROR), LoadStr(PW_MessageConfirm), p2) and (p2 <> '') then
      begin
...
function TZip32.GetExtrPassword: string;
var
  p1: string;
begin
  p1 := '';
  ...
  else if GetPassword(LoadZipStr(PW_Caption, RESOURCE_ERROR), LoadStr(PW_MessageEnter), p1) and (p1 <> '') then
    if GAssignPassword then
      FPassword := p1;
  Result := p1;
end;

function TZip32.GetPassword(DialogCaption, MsgTxt: string; var ResultStr: string): Boolean;
var
  OkClicked: Boolean;
  Pdlg: TPasswordDlg;
begin
  Pdlg := TPasswordDlg.CreateNew(Self{$IFDEF VERD4+}, 0{$ENDIF});
...
//      FOnMessage(Self, 0, 'Temporary zipfile: ' + TmpZipName);
      FOnMessage('Temporary zipfile: ' + TmpZipName);
  end
  else
  ...
  if (Uppercase(ExtractFileExt(FZipFileName)) = '.EXE') and (FSFXOffset = 0) and not FileExists(FZipFileName) then
  begin
    try
...
function TZip32.ExtractFileToStream(FileName: string): TZipStream;
begin
  ...
  FZipStream.Clear();
  ExtExtract(1, TStream(nil), False);
...
procedure TZip32.Extract();
begin
  ExtExtract(0, nil, False);
end;

{
procedure TZip32.ExtractFromStream( InStream: TStream; IsData: Boolean );
begin
   if Assigned( InStream ) then
      ExtExtract( 2, InStream, IsData );
end;
}

// UseStream =1 use outputstream, =2 use inputstream, =3 use both.
// IsData =false the stream is a zipfile, =true it's data only.

procedure TZip32.ExtExtract(UseStream: Integer; InStream: TStream; IsData: Boolean);
var
...
begin
  ...
  if FUnattended and (FPassword = '') and (AddEncrypt in FAddOptions) and
    not Assigned(FOnPasswordError) then
  begin
    FPasswordReqCount := 0;
  ...
  if DirExists(fExtrBaseDir) then
    SetCurrentDir(fExtrBaseDir);

  UnzDLLVers := UnzVers;

  if ((UseStream and 3) <> 0) and (UnzDLLVers < 160) then
  begin
    FUnzBusy := False;
    if AutoLoad then Unload_Unz_Dll();
  ...
  try
    try
      ...
      if ((not (UseStream and 1)) <> 0) or (((UseStream and 1) <> 0) and UnZipParms.up2.fUseOutStream) then
        fSuccessCnt := UnzDLLExec(Pointer(UnZipParms.up1));
         { Remove from memory if stream is not Ok. }
      if ((UseStream and 1) <> 0) and (FSuccessCnt <> 1) then
        FZipStream.Clear();
...
function TZip32.CopyFile(const InFileName, OutFileName: string): Integer;
const
...
function TZip32.EraseFile(const Fname: string; How: DeleteOpts): Integer;
var
  SHF: TSHFileOpStruct;
  DelFileName: string;
begin
...
function TZip32.MakeTempFileName(Prefix, Extension: string): string;
var
  Buffer: pChar;
  len: DWORD;
begin
...
function TZip32.SearchResDirEntry(const ResStart: pIRD; const entry: pIRDirE; Depth: Integer): DWord;
var
  data: pIRDatE;
begin
  Result := 0;
...
function TZip32.BrowseResDir(const ResStart: pIRD; const dir: pIRD; Depth: Integer): DWord;
var
  SingleRes: pIRDirE;
  i: Integer;
begin
...
function TZip32.LookForDirs(const SectionData: Pointer; const SectionVirtualStart: DWord;
  const SectionLen: Integer; const directories: pIDD): DWord;
var
  ResStart: Pointer;
  dirs: pIDD;
...
function TZip32.ReplaceIcon(SFXFile, SFXSize: Integer): Integer;
const
  SE_NoFoundError = -10;
...
function TZip32.ConvertSFX: Integer;
const
...
function TZip32.CopyBuffer(InFile, OutFile, ReadLen: Integer): Integer;
const
...
function TZip32.RWCentralDir(OutFile: Integer; EOC: ZipEndOfCentral; OffsetChange: Integer): Integer;
var
  CEH: ZipCentralHeader;
  i: Integer;
begin
...
function TZip32.ConvertZIP: Integer;
const
  SE_CreateError = -1; { error in open of outfile }
  SE_CopyError = -2; { read or write error during copy }
  SE_OpenReadError = -3; { error in open of infile }
...
function TZip32.CheckIfLastDisk(var EOC: ZipEndOfCentral; DoExcept: boolean): boolean;
var
  Sig: Cardinal;
...
procedure TZip32.Unload_Zip_Dll;
begin
  if ZipDllHandle <> 0 then
...
procedure TZip32.Unload_Unz_Dll;
begin
  if UnzDllHandle <> 0 then
...
procedure TZip32.DiskFreeAndSize(Action: Integer); // RCV150199
var
...
function TZip32.IsDiskPresent: Boolean;
var
...
function TZip32.CheckForDisk: Integer;
var
...
function TZip32.IsRightDisk(drt: Integer): Boolean;
var
  Ext: string;
begin
  Result := False;
...
procedure TZip32.GetNewDisk(DiskSeq: Integer);
var
  drt: Integer;
begin
  drt := DRIVE_REMOVABLE;
...
function TZip32.AppendSlash(sDir: string): string;
begin
...
function TZip32.ReplaceForwardSlash(aStr: string): string;
var
  i: Integer;
begin
...
function TZip32.ZipFormat: Integer;
var
  func: function(winh: HWND; var mythis: TZip32): Boolean; stdcall;
begin
  FFound := 0;
...
function TZip32.WriteSpan(InFileName, OutFileName: string): Integer;
var
  LOH: ZipLocalHeader;
...
begin
  ...
  try
    ...
    if Assigned(FOnMessage) then
      FOnMessage(LoadZipStr(GE_CopyFile, 'Copying: ') + LoadZipStr(DS_CopyCentral, 'Central directory'));
    if Assigned(FOnProgress) then
...
function TZip32.MakeString(Buffer: pChar; Size: Integer): string;
begin
...
procedure TZip32.WriteSplit(Buffer: pChar; Len: Integer; MinSize: Integer);
var
  Res, MaxLen: Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FDiskWritten := 0;
        FNewDisk := True;
            // If all this was on a HD then this won't be usefull but...
        Continue;
      end;
...
function TZip32.ReadSpan(InFileName: string; var OutFilePath: string): Integer;
var
  Buffer: array[0..BufSize - 1] of Char;
...
procedure TZip32.AllocSpanMem(TotalEntries: Integer);
var
  i: Integer;
begin
  MDZD := TList.Create;
...
procedure TZip32.DeleteSpanMem;
var
  i: Integer;
begin
...
function TZip32.FindZipEntry(Entries: Integer; Filename: string): Integer;
var
  k: Integer;
begin
...
procedure TZip32.WriteJoin(Buffer: pChar; BufferSize, DSErrIdent: Integer);
begin
...
procedure TZip32.RWSplitData(Buffer: pChar; ReadLen, ZSErrVal: Integer);
var
  SizeR, ToRead: Integer;
begin
  while ReadLen > 0 do
...
procedure TZip32.RWJoinData(Buffer: pChar; ReadLen, DSErrIdent: Integer);
var
...
constructor FormatThread.CreateFT(var MyParent: TZip32; CreateSuspended: Boolean);
begin
...
function TZip32.Rename(RenameList: TList; DateTime: Integer): Integer;
var
  EOC: ZipEndOfCentral;
  CEH: ZipCentralHeader;
  LOH: ZipLocalHeader;
...
function TZip32.CopyZippedFiles(DestZipMaster: TZip32; DeleteFromSource: boolean; OverwriteDest: OvrOpts): Integer;
var
  EOC: ZipEndOfCentral;
  CEH: ZipCentralHeader;
  OutFilePath: string;
...
function ArchiveFiles(filelist: TStringlist; zipfile: string; logfn: TLogEvent): boolean;
var
  curdirbuff: TByteArray;
begin
  {PW restore current directory after using zip DLL}
  GetCurrentDirectory(pred(sizeof(curdirbuff)), @curdirbuff);
  with TZip32.create(nil) do
  begin
    Unattended := true;
    OnMessage := logfn;
    Verbose := true;
    DLLDirectory := uSystemUtils.GetAztecInstallationFolder;
    Handle := 0;
    ErrCode := 0;
    FSpecArgs.assign(filelist);
    ZipfileName := zipfile;
    Add;
    result := (ErrCode = 0);
    free;
  end;
  SetCurrentDirectory(@curdirbuff);
end;
...
begin
  ...
  begin
    ...
    FSpecArgs.assign(filelist);
    ZipfileName := zipfile;
    {PW 22/10/01 Recurse from input path}
...
begin
  ...
  begin
    ...
    ErrCode := 0;
    ZipfileName := zipfile;
    ExtrBaseDir := destinationfolder;
    {PW 22/10/01 Extract with directories}
